 ...
 */
class FIMWithAprioriSuite extends FunSuite with LocalSparkContext {
  ...
  test("test FIM with Apriori")
  {
    ...
    assert(fimWithApriori.apriori(rdd,0.8,sc).length == 1)

    assert(fimWithApriori.apriori(rdd,0.7,sc).length == 1)

    assert(fimWithApriori.apriori(rdd,0.6,sc).length == 2)

    assert(fimWithApriori.apriori(rdd,0.5,sc).length == 18)

    assert(fimWithApriori.apriori(rdd,0.4,sc).length == 18)

    assert(fimWithApriori.apriori(rdd,0.3,sc).length == 54)

    assert(fimWithApriori.apriori(rdd,0.2,sc).length == 54)

    assert(fimWithApriori.apriori(rdd,0.1,sc).length == 625)
