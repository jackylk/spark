...
/**
 * scala test unit
 * using Practical Machine Learning Book data to test the FPGrowth algorithm
 */
class FPGrowthSuite  extends FunSuite with LocalSparkContext {
  ...
  test("test FIM with FPGrowth")
  {
    ...
    val target: (RDD[Array[String]], Double, SparkContext) => Array[(String, Int)] = FPGrowth.fpgrowth
